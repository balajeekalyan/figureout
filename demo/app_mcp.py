...
# Make event_mcp and roles importable from the examples directory
_MCP_DIR = Path(__file__).parent.parent / "examples" / "event-sports-booking" / "mcp-server"
sys.path.insert(0, str(_MCP_DIR))

from event_mcp import event_mcp, load_json  # noqa: E402
from roles import ROLES  # noqa: E402
from figureout import FigureOut  # noqa: E402
...
def get_engines(provider: str, model: str, api_key: str):
    """Return the three FigureOut engine variants used by this app."""
    common = dict(
        llm=provider,
        llm_version=model,
        roles=ROLES,
    ...
    with st.chat_message("assistant"):
        with st.spinner("Calling tools…"):
            try:
                ...
                # First classify with the general engine to know which engine to use
                # We don't know the role yet, so run with None (auto-route via discovery engine)
